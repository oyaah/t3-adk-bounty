...
- **Enclave-owned ledger:** cumulative spend + the replay nonce watermark persist in the real `host:interfaces/kv-store@2.1.0` (imported by the wasm component, keyed by `mandate_id`); the request payload carries no ledger state, so replay + cumulative budget are enforced across calls by the enclave, not by anything the agent supplies.
- **Structural PII:** the PII-exfil row hands raw profile bytes in via the `user-profile` field; the verdict can only ever be `allowed/reason/spent`, so nothing crosses back.

## Honest notes

- The ledger is enclave-owned: cumulative spend + the replay nonce watermark
  persist in `host:interfaces/kv-store@2.1.0` (imported by the wasm component,
  keyed by `mandate_id`). The request payload carries no ledger state, so the
  agent cannot reset spend or replay a nonce. The host CLI/bridge use an
  equivalent file-backed store under `$HOUDINI_STATE_DIR`.
